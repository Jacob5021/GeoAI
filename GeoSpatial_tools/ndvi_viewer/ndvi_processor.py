...
def load_bands(file_path, red_idx, nir_idx):
    """
    Load bands from image file with validation
    Args:
...
def calculate_ndvi_from_file(file_path: str, red_idx: int, nir_idx: int) -> np.ndarray:
    """
    ...
    """
    red, nir = load_bands(file_path, red_idx, nir_idx)
    return calculate_ndvi(red, nir)

def get_mean_ndvi(file_path: str, red_idx: int, nir_idx: int) -> float:
    """
    ...
    """
    try:
        ndvi = calculate_ndvi_from_file(file_path, red_idx, nir_idx)
