...
def data_uploader():
    ...
    with st.expander("Upload Files", expanded=True):
        files = st.file_uploader(
            ...
            help="Supported: GeoTIFF, JPG, PNG, CSV, GeoJSON, Shapefile, KML"
        )
        ...
        if files:
            for file in files:
                ...
                if file_type in allowed_extensions:
                    ...
                    # Auto-tag for flood modules
                    fname = file.name.lower()
                    if "dem" in fname:
                        st.session_state.uploaded_files["dem"] = file
                    elif "precip" in fname or "rain" in fname:
                        st.session_state.uploaded_files["precip"] = file
                    else:
                        st.session_state.uploaded_files.setdefault(file_type, []).append(file)
